import React, { useEffect } from "react"
import Fab from "./fab"
import "./audio-recorder.scss"
import {
  SpeechConfig,
  AudioConfig,
  SpeechRecognizer,
} from "microsoft-cognitiveservices-speech-sdk"

import Microphone from "../images/mic.inline.svg"

const AudioRecorder = ({ onRecordStart, onRecordEnd }) => {
  const onClick = () => {
    onRecordStart()

    let speechConfig

    if (authorizationToken) {
      speechConfig = SpeechConfig.fromAuthorizationToken(
        authorizationToken,
        serviceRegion
      )
    } else {
      speechConfig = SpeechConfig.fromSubscription(
        subscriptionKey,
        serviceRegion
      )
    }

    speechConfig.speechRecognitionLanguage = speechRecognitionLanguage

    let audioConfig = AudioConfig.fromDefaultMicrophoneInput()

    recognizer = new SpeechRecognizer(speechConfig, audioConfig)

    recognizer.recognizeOnceAsync(
      function(result) {
        console.log(result)
        const { text } = result

        // window.tag = ""

        if (text.includes("thirsty")) {
          window.tag = "thirsty"
        }

        if (text.includes("hungry")) {
          window.tag = "hungry"
        }

        if (text.includes("tired")) {
          window.tag = "tired"
        }

        recognizer.close()
        recognizer = undefined

        onRecordEnd()
      },
      function(err) {
        console.log(err)

        recognizer.close()
        recognizer = undefined
      }
    )
  }

  let authorizationToken
  let recognizer
  const subscriptionKey = "da482cf4bb1f4c1784fe12b12ff81d88"
  const serviceRegion = "francecentral"
  const speechRecognitionLanguage = "en-US"

  return (
    <Fab
      className="audio-recorder__fab"
      icon={
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
          height="24"
          viewBox="0 0 24 24"
        >
          <path d="M12 14c1.66 0 2.99-1.34 2.99-3L15 5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm5.3-3c0 3-2.54 5.1-5.3 5.1S6.7 14 6.7 11H5c0 3.41 2.72 6.23 6 6.72V21h2v-3.28c3.28-.48 6-3.3 6-6.72h-1.7z" />
          <path d="M0 0h24v24H0z" fill="none" />
        </svg>
      }
      onClick={onClick}
    />
  )
}

export default AudioRecorder
